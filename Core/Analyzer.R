...
ggplot(Rust,aes(x=converted,fill = Degree))+geom_histogram(binwidth=10, colour="white",position = 'dodge')+scale_x_date(labels = date_format("%d-%b"), breaks =seq(min(Rust$converted)-1, max(Rust$converted)+1, 10))+theme(axis.text.x = element_text(angle = 90, hjust = 1,size = 10))+xlab('Date')+ylab('Frequency')
dev.off()
png('FrequencyAnalysis_Decision.png')
ggplot(Rust,aes(x=converted,fill = decision))+geom_histogram(binwidth=10, colour="white",position = 'dodge')+scale_x_date(labels = date_format("%d-%b"), breaks =seq(min(Rust$converted)-1, max(Rust$converted)+1, 10))+theme(axis.text.x = element_text(angle = 90, hjust = 1,size = 10))+xlab('Date')+ylab('Frequency')
dev.off()
